...
const CategoryCard = ({ image, title, className = '', onClick }) => {
  return (
    <button
      ...
      `.trim().replace(/\s+/g, ' ')}
    >
      <div className="w-full h-full flex flex-row items-center gap-3 p-3 lg:p-4 min-h-[72px]">
        {/* Текст слева-сверху */}
        ...
        {/* Иконка/картинка справа — 60–70px */}
        {image && (
          <div className="flex-shrink-0 flex items-center justify-center w-14 h-14 lg:w-16 lg:h-16">
            <img
              src={image}
              alt={title?.replace('\n', ' ')}
